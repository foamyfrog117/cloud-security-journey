...
.DATE
    November 2025
.VERSION
    0.4 - Added S3 SSL/TLS enforcement check
#>
...
function Get-S3SecurityFindings {
    ...
    foreach ($bucket in $buckets) {
        ...
        try {
            ...
            # COMMENT: Check if the returned policy string is not null or empty
            # before trying to convert it from JSON.
            if (-not [string]::IsNullOrEmpty($policy.Policy)) {
                ...
                # COMMENT: Force the 'Statement' property to be an array
                # This handles policies with just one statement.
            ...
            # If $policy.Policy was null, we just skip the 'if' block,
            # and $hasSSLEnforcement remains $false, which is correct.
        }
        catch {
        ...
        # COMMENT: Updated this logic to include the new SSL check.
        if ($isPublic) {
            $severity = "HIGH"
        ...
        # --- NEW SEVERITY LOGIC ---
        elseif (-not $hasSSLEnforcement) {
            $severity = "MEDIUM"
            $recommendation = "Add a bucket policy to deny non-SSL (HTTP) access"
        }
        # --- END OF NEW LOGIC ---
        ...
        # COMMENT: Added '.SSLEnforced' to the finding object for reporting.
        $finding = [PSCustomObject]@{
            ...
            SSLEnforced = if ($hasSSLEnforcement) { "YES" } else { "NO" } # <-- NEW PROPERTY
            Severity = $severity
...
Write-Host "║   AWS Security Baseline Checker v0.4      ║" -ForegroundColor Green
