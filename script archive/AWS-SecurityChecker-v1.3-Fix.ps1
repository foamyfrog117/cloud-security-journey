...
function Get-S3SecurityFindings {
    ...
    foreach ($bucket in $buckets) {
        ...
        try {
            # --- START: SSL BUG FIX ---
            # We proved the 'aws s3api' command works, while 'Get-S3BucketPolicy' fails.
            # We will now use the CLI command for this check.
            $cliPolicyJson = aws s3api get-bucket-policy --bucket $bucket.BucketName --region $bucketRegion --query Policy --output text 2>&1
            
            # Check if the CLI returned an actual JSON policy '{...}' and not an error/empty string
            if ($null -ne $cliPolicyJson -and $cliPolicyJson.StartsWith("{")) {
                
                # The CLI command gives us the raw JSON string.
                $policyJson = $cliPolicyJson | ConvertFrom-Json
                $statements = @($policyJson.Statement)

                $requiresSSL = $statements | Where-Object {
                    $_.Effect -eq "Deny" -and
                    # Check if the SecureTransport property exists and is set to "false" (as a string)
                    # This is safer than checking the boolean $false
            ...
            # This 'catch' block will catch any *other* errors in the logic
            Write-Host "  DEBUG (PS): SSL Policy check failed with: $_" -ForegroundColor Magenta
            $hasSSLEnforcement = $false
        }
...
Write-Host "║   AWS Security Baseline Checker v1.3      ║" -ForegroundColor Green
Write-Host "║   (SSL Bug Fix)                           ║" -ForegroundColor Green
