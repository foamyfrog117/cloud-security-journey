...
function Get-S3SecurityFindings {
    ...
    foreach ($bucket in $buckets) {
        ...
        try {
            # MODIFIED: Added -ErrorAction Stop to force errors into the 'catch' block
            $policy = Get-S3BucketPolicy -BucketName $bucket.BucketName -Region $bucketRegion -ErrorAction Stop
            
            # MODIFIED: Added new debug line to see the raw policy string
            Write-Host "  DEBUG: Raw policy string is: '$($policy.Policy)'" -ForegroundColor DarkCyan

            if (-not [string]::IsNullOrEmpty($policy.Policy)) {
                $policyJson = $policy.Policy | ConvertFrom-Json
                $statements = @($policyJson.Statement)

                # --- START: ADDED DEBUG LOGGING (from W46 Plan) ---
                Write-Host "  DEBUG: Found $($statements.Count) statement(s)" -ForegroundColor Magenta
                foreach ($stmt in $statements) {
                    Write-Host "  DEBUG: Effect=$($stmt.Effect)" -ForegroundColor Magenta
                    # Corrected the property access logic for safety
                    if ($null -ne $stmt.Condition.Bool) {
                        Write-Host "  DEBUG: SecureTransport=$($stmt.Condition.Bool.'aws:SecureTransport')" -ForegroundColor Magenta
                    } else {
                        Write-Host "  DEBUG: Statement has no 'Condition.Bool' property." -ForegroundColor DarkGray
                    }
                }
                ...
                $requiresSSL = $statements | Where-Object {
                    ...
                    $_.Condition.Bool.'aws:SecureTransport' -eq "false" # <-- This is the likely bug
                }
            ...
            # This 'catch' block will now trigger on "NoSuchBucketPolicy" errors
...
Write-Host "║   AWS Security Baseline Checker v1.1      ║" -ForegroundColor Green
