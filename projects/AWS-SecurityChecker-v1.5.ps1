...
.DATE
    November 18, 2025
.VERSION
    1.5 - Added EC2 Security Checks (Security Groups + Key Pairs)
...
Write-Host "║   AWS Security Baseline Checker v1.5      ║" -ForegroundColor Green
Write-Host "║   (EC2 Security Expansion)                ║" -ForegroundColor Green
#>
...
#=======================================================================
# EC2 SCANNER FUNCTION
#=======================================================================
function Get-EC2SecurityFindings {
    Write-Host "`n=== Checking EC2 Security ===" -ForegroundColor Cyan

    # --- CHECK 1: SECURITY GROUPS (Firewall Check) ---
    try {
        $securityGroups = Get-EC2SecurityGroup -Region "us-east-2" -ErrorAction Stop
        Write-Host "  > Scanned $($securityGroups.Count) Security Groups." -ForegroundColor Gray
        
        foreach ($sg in $securityGroups) {
            foreach ($perm in $sg.IpPermissions) {
                # Check if the rule allows ALL traffic (0.0.0.0/0)
                if ($perm.IpRanges.CidrIp -contains "0.0.0.0/0") {
                    
                    # Check if it exposes SSH (22)
                    if (22 -ge $perm.FromPort -and 22 -le $perm.ToPort) {
                        $script:results += [PSCustomObject]@{
                            ResourceType = "EC2 Security Group"
                            ResourceName = $sg.GroupName
                            Region       = "us-east-2"
                            Severity     = "HIGH"
                            Recommendation = "Remove 0.0.0.0/0 from SSH (Port 22)"
                            CheckedDate  = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
                            # Fill N/A for S3-specific columns to keep CSV clean
                            PublicAccess = "N/A"; Encrypted = "N/A"; Versioned = "N/A"; 
                            MFADelete = "N/A"; Logging = "N/A"; SSLEnforced = "N/A"
                        }
                    }

                    # Check if it exposes RDP (3389)
                    if (3389 -ge $perm.FromPort -and 3389 -le $perm.ToPort) {
                        $script:results += [PSCustomObject]@{
                            ResourceType = "EC2 Security Group"
                            ResourceName = $sg.GroupName
                            Region       = "us-east-2"
                            Severity     = "HIGH"
                            Recommendation = "Remove 0.0.0.0/0 from RDP (Port 3389)"
                            CheckedDate  = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
                            PublicAccess = "N/A"; Encrypted = "N/A"; Versioned = "N/A"; 
                            MFADelete = "N/A"; Logging = "N/A"; SSLEnforced = "N/A"
                        }
        ...
        Write-Host "[ERROR] EC2 Security Group Check failed: $_" -ForegroundColor Red
    }

    # --- CHECK 2: KEY PAIRS (Identity Check) ---
    try {
        $keyPairs = Get-EC2KeyPair -Region "us-east-2" -ErrorAction Stop
        Write-Host "  > Scanned $($keyPairs.Count) Key Pairs." -ForegroundColor Gray
        $ninetyDaysAgo = (Get-Date).AddDays(-90)

        foreach ($key in $keyPairs) {
            # AWS returns CreateTime as a string usually, but AWS.Tools might auto-convert.
            # We cast to [datetime] to be safe.
            $keyAge = [datetime]$key.CreateTime

            if ($keyAge -lt $ninetyDaysAgo) {
                $script:results += [PSCustomObject]@{
                    ResourceType = "EC2 Key Pair"
                    ResourceName = $key.KeyName
                    Region       = "us-east-2"
                    Severity     = "MEDIUM"
                    Recommendation = "Key is older than 90 days. Rotate key pair."
                    CheckedDate  = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
                    PublicAccess = "N/A"; Encrypted = "N/A"; Versioned = "N/A"; 
                    MFADelete = "N/A"; Logging = "N/A"; SSLEnforced = "N/A"
                }
        ...
        Write-Host "[ERROR] EC2 Key Pair Check failed: $_" -ForegroundColor Red
    }
}
